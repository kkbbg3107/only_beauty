...
def test_salary_breakdown():
    ...
    # 美容師 (底薪28590 + 加班1564 = 30154)
    data[8][10] = "美容師小美"
    data[8][11] = 30154     # L9: 底薪+加班費合計
    data[8][12] = 5000      # M9: 手技獎金
    
    # 護理師 (底薪28590 + 加班2588 = 31178)
    data[8][16] = "護理師小雅"
    data[8][17] = 31178     # R9: 底薪+加班費合計
    data[8][18] = 8000      # S9: 手技獎金
    
    # 櫃檯 (底薪28590 + 加班2464 = 31054)
    data[11][16] = "櫃檯小君"
    data[11][17] = 31054    # R12: 底薪+加班費合計
    ...
    for staff in staff_data:
        ...
        print(f"  Excel中的底薪+加班費: {staff['base_salary']:,}")
    ...
    for name, salary_data in individual_staff_salaries.items():
        ...
        print(f"  底薪+加班費小計: {base_salary + overtime_pay:,}")
        print(f"  手技獎金: {hand_skill:,}")
        
        if position in ['美容師', '護理師']:
            print(f"  團體業績獎金: {team_perf:,}")
            print(f"  團體消耗獎金: {team_cons:,}")
        ...
        # 驗證計算
        expected_total = (base_salary + overtime_pay + hand_skill + team_perf + 
                         team_cons + high_target + license_allow + full_attend + 
                         rank_bonus + pos_allow)
        print(f"  驗證計算: {expected_total:,} {'✓' if expected_total == total else '✗'}")
    
    print("\n驗證預期加班費:")
    print("美容師加班費應為: 30,154 - 28,590 = 1,564")
    print("護理師加班費應為: 31,178 - 28,590 = 2,588") 
    print("櫃檯加班費應為: 31,054 - 28,590 = 2,464")

if __name__ == "__main__":
    test_salary_breakdown()
